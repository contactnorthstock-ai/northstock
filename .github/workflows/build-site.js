...
function main() {
  ...
  // ── Inject Finnhub API key from environment variable ──────────
  // Key is stored in GitHub Secret FINNHUB_API_KEY
  // HTML files use placeholder __FINNHUB_KEY__ which gets replaced here
  const finnhubKey = process.env.FINNHUB_API_KEY || '';
  if (!finnhubKey) {
    console.warn('⚠️  FINNHUB_API_KEY not set — prices may not load');
  } else {
    console.log('🔑 Injecting Finnhub API key...');
  }

  function injectKey(html) {
    return html.replace(/__FINNHUB_KEY__/g, finnhubKey);
  }

  // Also inject into index.html already written
  const indexPath = path.join(distDir, 'index.html');
  if (fs.existsSync(indexPath)) {
    fs.writeFileSync(indexPath, injectKey(fs.readFileSync(indexPath, 'utf8')));
  }

  // Copy all additional pages with key injection
  const pages = [
    'earnings.html',
    'dividends.html',
    'screener.html',
